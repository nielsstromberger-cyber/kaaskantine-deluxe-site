...
import { Package, Clock, User, Phone, Mail, RefreshCcw, Search } from "lucide-react";
...
async function fetchOrders(): Promise<OrderRow[]> {
  const { data, error } = await supabase
    .from("orders")
    .select(
      "id, order_number, customer_name, customer_email, customer_phone, pickup_time, status, total_cents, notes, created_at",
    )
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-semibold text-primary">Bestellingen</h1>
          <p className="mt-1 text-sm text-muted-foreground">Live inkomende bestellingen.</p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="mt-6 grid gap-4">
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            ...
            <p className="mt-3 text-muted-foreground">Geen bestellingen.</p>
          </div>
        )}
        {filtered.map((o) => (
          <OrderCard
